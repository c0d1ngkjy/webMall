<template>
  <q-page class="bg-secondary">
    ...
    <div :class="isMobile ? 'q-px-sm' : 'q-px-xl'" class="q-py-md q-px-md row justify-between">
      <q-btn @click="$router.push('/book/uploadbook')" unelevated class="bg-green-6 text-white text-lowercase" size="sm">upload new book</q-btn>
      <div>
    ...
    <q-list v-if="listView == true" separator :class="isMobile ? 'q-px-lg' : 'q-px-xl'" class="q-py-md">
    ...
    <div class="row justify-start q-gutter-md q-py-md"  v-if="listView == false" :class="isMobile ? 'q-px-sm' : 'q-px-xl'">
      <q-card @click="$router.push(`book/info/${book.bookId}`)" flat :style="isMobile ? 'width:40vw' : 'width: 11vw'" class="column justify-between bg- content-center items-center q-pa-md cursor-pointer" v-for="book in books" :key="book">
        <div>
          <q-img :width="isMobile ? '30vw' : '4vw'" :src="book.bookData.bookCoverRef ? book.bookData.bookCoverRef : bookCoverPlaceHolderUrl" alt="book cover placeholder img"></q-img>
        </div>
